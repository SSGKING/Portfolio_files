...
def _readFile(fname, missingOK=False):
    possibilities = [fname, 
        os.path.abspath(os.path.join(os.path.dirname(__file__),"..","src","shaders",fname)),
        os.path.abspath(os.path.join(os.path.dirname(__file__),"..","src","py","shaders",fname)),
        os.path.abspath(os.path.join(os.path.dirname(__file__),"..","src","py",fname)),
        ...
        os.path.abspath(os.path.join("shaders",fname)), 
    ]
    found=[]
    shaderdata = None
...
def _compile(S,shaderType,prog):
    ...
    glCompileShader(s)
    _getLog( S.filename, "uniforms.txt", "Compiling "+shaderTypeString+" "+S.filename+": ",
        glGetShaderInfoLog, s)
    tmp = array.array("I",[0])
    glGetShaderiv( s, GL_COMPILE_STATUS, tmp )
    if not tmp[0]:
        raise ShaderError("Cannot compile!")
